...
- Two-hop flow: build on a feature branch → PR to `develop` (deploys staging) → PR `develop` to `main` (deploys production). Never commit directly to `develop` or `main` — both are protected and require the CI checks (`check`, `build`) to pass on the PR. A merge to `main` doesn't always auto-redeploy prod — verify the deployed commit and hit Redeploy in Coolify if it lags. `NEXT_PUBLIC_*` vars inline at build, so set them (and mark build-time) before the prod build, not after.
- **The site is live** (launched 2026-07-12). The build phases (00 groundwork → 05 go-live) are done; work is now in Phase 06 (Care): analytics, SEO/structured data, post-launch fixes, dependency upkeep. Session work still ticks the relevant phase note in `30 - Projects/Personal Site/Phases/`, and new scope goes to the project board (below) rather than expanding the task in hand.
- **Backlog lives on the GitHub Project board**, not in vault notes: **richardkern.nz website** (`gh project` owner `richardkern`, project #3), backed by issues in this repo. It is the source of truth for concrete, actionable-but-deferred work. When such a task comes up, add a card (new items default to the Backlog column; columns are Backlog / Ready / In progress / In review / Done, plus Priority P0–P2 and Size XS–XL) and leave a short pointer in the relevant vault note rather than re-describing it there.
